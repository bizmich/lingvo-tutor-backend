...
import { type IUpdateWord, wordTable } from "../models/word.schema.ts";
import {
	createWordService,
...
import type { IAuthRequest, IRequest } from "../shared/types/index.ts";
...
export const createWord = async (request: IAuthRequest, response: Response) => {
	try {
		const _userId = request.user.id;
		const data = await createWordService({
			...request.body,
			userId: _userId,
		});
